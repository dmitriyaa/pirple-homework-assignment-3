...
// Log the user out then redirect
    ...
    for(var i = 0; i < allForms.length; i++){
        ...
        for(var i = 0; i < elements.length; i++){
          if(elements[i].type !== 'submit'){
            if (elements[i].classList.contains('type-number')) {
              var valueOfElement = parseInt(elements[i].value, 10);
            } else {
  ...
  if (email) {
    ...
    app.client.request(undefined, 'api/users', 'GET', queryStringObject, undefined, (statusCode, responsePayload) => {
      if (statusCode == 200) {
        ...
        document.querySelector('#accountEdit1 #streetAddress').value = responsePayload.streetAddress;

        // Put the hidden email field into both forms
        const hiddenEmailInput = document.querySelectorAll('input.hiddenEmailInput');
          ...
          <div class="col-4 mb-3">
            <div class="card"">
              <div class="card-body">
                ...
                <p>price/pizza: <strong>${item.price}</strong>CZK</p>
                <form action="/api/shopping-cart" method="POST" id="form-cart-${item.name}">

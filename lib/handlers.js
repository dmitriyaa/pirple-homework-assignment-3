 ...
 */
 ...
 handlers.index = (data, callback) => {
   ...
   if (data.method == 'get') {
     ...
     const templateData = {
       ...
       'head.description': 'We offer a fast and simple pizza delivery',
       'body.class': 'index'
     };
 ...
 handlers.foodMenu = (data, callback) => {
   // Reject any request that isn't a GET
   if (data.method == 'get') {
     // Prepare data for interpolation
     const templateData = {
       'head.title': 'Menu',
       'head.description': 'List of all courses we have',
       'body.class': 'foodMenu'
     };

     // Read in a template as a string
     helpers.getTemplate('foodMenu', templateData, (err, str) => {
       if (!err && str) {
         // Add the universal header and footer
         helpers.addUniversalTemplates(str, templateData, (err, str) => {
           if (!err && str) {
 ...
 handlers.cart = (data, callback) => {
  ...
  // Orderd
  handlers.orderd = (data, callback) => {
    // Reject any request that isn't a GET
    if (data.method == 'get') {
      // Prepare data for interpolation
      const templateData = {
        'head.title': 'Orderd',
        'head.description': 'Your order was successfully completed',
        'body.class': 'orderd'
      };

      // Read in a template as a string
      helpers.getTemplate('orderd', templateData, (err, str) => {
        if (!err && str) {
          // Add the universal header and footer
          helpers.addUniversalTemplates(str, templateData, (err, str) => {
            if (!err && str) {
